        ...
        data.map((item) => (
          <View
            ...
            accessibilityLabel={t("detail.accessibility.breakdown_item", {
              defaultValue: `${item.name}. ${t("detail.breakdown.transaction_count", { count: item.transactionCount, defaultValue: `${item.transactionCount} transactions` })}. ${formatLocalizedMoneyAmount({ amount: item.amount, currency })}. ${Math.round(item.percentage)} percent.`,
              name: item.name,
              countLabel: t("detail.breakdown.transaction_count", {
                count: item.transactionCount,
                defaultValue: `${item.transactionCount} transactions`,
              }),
              amount: formatLocalizedMoneyAmount({ amount: item.amount, currency }),
          ...
          >
            ...
            <View className="items-end ps-3">
              <Text className="text-sm font-medium text-text-primary dark:text-text-primary-dark">
                {formatLocalizedMoneyAmount({ amount: item.amount, currency })}
              </Text>
