...
describe("068 Metals domain migration and persisted models", () => {
  ...
  it("keeps the financial foundation before Metals and reserves 069", () => {
    expect(source("supabase/migrations/067_financial_action_foundation.sql")).toContain(
      "financial_action_groups"
    );
    expect(() => source(migrationPath)).not.toThrow();
    expect(() => source("supabase/migrations/069_account_financial_effects.sql")).toThrow();
  });
  ...
  it("creates owner-scoped projections and immutable evidence with canonical revisions", () => {
    ...
    expect(sql).toMatch(/unique\s*\(\s*user_id\s*,\s*action_id\s*,\s*role\s*\)/i);
    expect(sql).toMatch(/9223372036854775807/);
    expect(sql).toMatch(/expected_holding_revision[\s\S]*metal_action_evidence/i);
    expect(source("supabase/migrations/067_financial_action_foundation.sql")).not.toContain(
  ...
  it("enforces RLS, parent ownership, sync columns, and query indexes", () => {
    ...
    for (const table of [
      ...
      expect(sql).toMatch(new RegExp(`${table}[\\s\\S]*\\(SELECT auth\\.uid\\(\\)\\)`, "i"));
    }
    expect(sql).toMatch(
    ...
    expect(sql).toMatch(/join\s+public\.asset_metals[\s\S]*metal\.asset_id\s*=\s*asset\.id/i);
    ...
    expect(sql).toMatch(/metal_holding_states[\s\S]*user_id[\s\S]*status[\s\S]*deleted/i);
