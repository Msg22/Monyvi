...
- [ ] T027 [P] After T025, write failing local/RPC/sync tests for expected account revision, owner-scoped idempotent CAS, fixed lock order, identical replay, hash mismatch, rollback, dedicated sync, failure propagation, and exact-once compensation in `apps/mobile/__tests__/services/account-financial-action-protocol.integration.test.ts` and `supabase/tests/account_financial_effects_test.sql`
...
- [ ] T037 [P] Write failing reconciliation/sync/rate/metadata tests for stale versus validation/incomplete server rejection, hash-mismatch non-retry, incomplete/foreign/missing canonical evidence, restart, action lock, exact-once holding restore or safe rollback, metadata LWW, pull/push failure, protected fragments, and no watermark advance in `apps/mobile/__tests__/services/metals-reconciliation-sync-rate.integration.test.ts`
...
- [ ] T042 Create `supabase/migrations/068_metals_domain.sql` with exact guarded backfill, states/events/references/observations, RLS, constraints, indexes, action-ID links, and expected holding revision stored/enforced only on Metals event/evidence for owner-scoped CAS that rejects account effects; run `npm run db:migrate` and update only Metals schema/migrations/generated types/persisted-field models/registrations under `packages/db/src/`
...
- [ ] T044 Implement exact-once stale-winner compensation and server-rejection recovery, incomplete retry/locking, immutable rate references, and name/notes-only LWW in `apps/mobile/services/metal-reconciliation-service.ts`, `apps/mobile/services/metal-rate-reference-service.ts`, and `apps/mobile/services/metal-metadata-service.ts`
...
- [ ] T152 Verify exact `24K · 999` math, Home totals/shares, Add preview, sale fee/result, account revision/effects, net-worth reconciliation, analytics exclusion, exact-once compensation, and snapshot immutability in `specs/035-metals-module-redesign/financial-verification.md`
