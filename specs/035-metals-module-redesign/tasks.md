...
- [ ] T042 Create `supabase/migrations/068_metals_domain.sql` with exact guarded backfill, states/events/role-unique references/observations, nullable current acquisition-action linkage with no invented legacy action, RLS, constraints, indexes, action-ID links, and required unique `(user_id, action_id)` on `metal_action_evidence` for exactly one owner-scoped evidence row per action; store/validate every holding revision as canonical unsigned-integer string at local/RPC boundaries and `bigint` remotely; enforce expected holding revision only on Metals event/evidence for owner-scoped CAS that rejects account effects; run `npm run db:migrate` and update only Metals schema/migrations/generated types/persisted-field models/registrations under `packages/db/src/`
