...
Status: isolated deterministic Green; shared route/registry/translations,
...
| D10 | FR-036, FR-039, FR-063–FR-065, FR-091–FR-098, SC-002, SC-007, SC-027–SC-030 | SQLite exact group/state/revision/history/no-account test, including predecessor-less revision zero | Manual offline journey until a controllable offline fixture exists | Green through injected registry; production registry integration open |
...
- Focused Jest: `2` suites, `45` tests passed.
- Focused coverage: `97.50%` statements, `93.01%` branches, `96.00%`
  functions, `99.11%` lines across the Dispose service, hook, and component.
  The command service is `100%` for statements, branches, functions, and lines.
...
- The shared default `metals.dispose/v1` registry requires a predecessor event.
  The revision-zero migration contract permits an Active migrated holding with
  no predecessor. Deterministic coverage uses a test-only registry variant;
  production revision-zero support remains a shared-registry integration gate.
