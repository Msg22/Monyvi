...
POST-only ingestion, response-time capture, exact USD identity spellings, required
metal-command calendar evidence, and unique migration prefixes. These are not
instructions to perform unavailable lifecycle actions in the app.
...
history must not be reset or have history rewritten without the tester's approval.
