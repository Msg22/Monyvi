...
| -------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------ |
| US5-NC-M01     | Active Gold holding, purchase cost EGP 151,278.20, fresh exact terminal references | Open Sell, enter EGP 170,000.00 gross and EGP 500.00 fee                              | Live summary shows EGP 169,500.00 net and EGP 18,221.80 profit; whole holding becomes Sold after direct submit | Partial    | Service/UI Green; Maestro authored but blocked         |
...
| US5-NC-M03     | Same holding                                                                       | Enter negative fee, fee above gross, zero gross, future date, or date before purchase | Field-specific error blocks persistence and is exposed accessibly                                              | Partial    | Service/UI Green; device focus recovery pending        |
...
| US5-NC-M05     | Active holding, device offline                                                     | Record sale, return to portfolio/history, restart app                                 | Local action completes without network; holding remains Sold after restart and appears in History              | Partial    | SQLite restart Green; live route/Maestro blocked       |
...
| US5-CREDIT-M01 | Any account fixture before full #242 gate                                          | Render isolated Sell screen                                                           | `Receive money in` is visibly unavailable; sale without credit remains enabled                                 | Partial    | Isolated UI Green; live route blocked                  |
...
The isolated Sell implementation deliberately has no Expo route. Runtime and
Maestro execution resume only after the shared action adapter can create the
approved envelope, shared copy is registered, and a deterministic Sell fixture
is owned by the integration lane. No dead route or runtime fallback copy is used
to overstate completion.
