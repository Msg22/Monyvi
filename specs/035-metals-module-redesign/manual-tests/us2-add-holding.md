...
counterpart: `metal-holding-form-validation.test.ts`,
`add-metal-holding-command-service.integration.test.ts`,
`metal-unusual-value-policy.test.ts`, `useAddMetalHoldingForm.test.tsx`,
...
| ------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ----------------------------------- |
| US2-M01 | Fresh Gold rate; English LTR                      | Enter all required Gold facts, optional Coin and Arabic/emoji note                                     | Same form normalizes input, shows chosen purity/factor and compact preview, then saves with direct Add holding.                                       | Unit + UI + Maestro               | T076–T078/T081/T082 Green; T079 unexecuted |
...
| US2-M03 | Fresh rate; decimal-comma input                   | Enter decimal-comma weight and price                                                                   | Values normalize once, live preview matches submitted canonical decimals.                                                                             | Unit + UI + Maestro               | Unit/UI Green; T079 unexecuted      |
...
| US2-M07 | Gold >= 1,000g, Silver >= 10,000g, or purchase >= EGP 10,000,000 equivalent | Submit at/above each inclusive boundary before, then after, acknowledgment | First submission stays in form with an explicit soft typo warning; acknowledgment permits direct Add. Latest usable cached FX may evaluate purchase amount, stale FX retains disclosure, and unavailable truthful FX never invents a conversion or blocks Add. | Unit + UI + Maestro | Policy/Hook/UI Green; T079 unexecuted |
| US2-M08 | Missing/invalid rate or offline cache             | Complete valid facts and inspect preview, then submit                                                  | Preview says valuation unavailable; holding facts remain complete, save locally, and later sync is pending rather than blocking.                      | Unit + integration + UI + Maestro | Unit/integration/UI Green; T079 unexecuted |
| US2-M09 | Offline valid fixture                             | Add holding, restart app, reopen portfolio                                                             | One locally saved holding remains with exact acquisition facts and no partial record.                                                                 | Integration + Maestro + device    | SQLite restart Green; Maestro/device pending |
| US2-M10 | Pending local writer / injected local failure     | Repeatedly tap Add, then retry after failure                                                           | Pending state prevents duplicate/exit; failure preserves input and produces no partial holding; retry yields one outcome.                             | Integration + UI + Maestro        | Integration/Hook/UI Green; Maestro pending |
