...
const PURCHASE_AT_SALE_EGP_RATE = directCurrencyRate(
  "terminal_purchase_currency",
  "currency:EGP",
  "0.25"
);
const PROCEEDS_EGP_RATE = directCurrencyRate(
...
const VALUATION_SAR_RATE = directCurrencyRate(
  "current_purchase_currency",
  "currency:SAR",
  "0.25"
);
const SALE_SILVER_RATE = metalRate("terminal_metal", "metal:SILVER", "3");
const PURCHASE_AT_SALE_SAR_RATE = directCurrencyRate(
  "terminal_purchase_currency",
  "currency:SAR",
  "0.25"
);
const PROCEEDS_SAR_RATE = directCurrencyRate(
  "terminal_proceeds_currency",
  "currency:SAR",
  "0.25"
);
...
describe("current and realized Metals attribution", () => {
  ...
  it("keeps current combined P/L when recorded current facts suffice but historical breakdown facts are missing", () => {
    ...
    expect(
      ...
      value: {
        combinedDecimal: "85",
        consumedRateReferences: [VALUATION_METAL_RATE, VALUATION_EGP_RATE],
        breakdown: {
          available: false,
          reasons: [
  ...
  it("keeps realized combined P/L when recorded sale facts suffice but detailed reference facts are missing", () => {
    ...
    expect(
      ...
      value: {
        combinedDecimal: "105",
        consumedRateReferences: [PURCHASE_AT_SALE_EGP_RATE, PROCEEDS_EGP_RATE],
        canonicalGrossProceedsDecimal: "150",
        canonicalFeesDecimal: "10",
        netProceedsDecimal: "140",
        breakdown: {
  ...
  it.each([
    ...
    (_reason, mismatchedTerminalRates) => {
      ...
      expect(
        result.value.consumedRateReferences.every(
  ...
  it.each([
    ...
    [
      "negative gross",
      { grossProceedsDecimal: "-1" },
      "gross_proceeds_unavailable",
    ],
    [
      "invalid gross",
      { grossProceedsDecimal: "not-a-decimal" },
      "gross_proceeds_unavailable",
    ],
    [
      "subminor gross",
      { grossProceedsDecimal: "100.001" },
      "gross_proceeds_unavailable",
    ],
    ...
    "rejects %s before emitting realized P/L",
  ...
  it("normalizes reciprocal FX orientations to the same USD-per-unit factor", () => {
    ...
    expect(
      normalizeUsdPerUnitRate(
        directCurrencyRate("current_purchase_currency", "currency:EGP", "0.02")
      )
    ).toEqual({
      available: true,
      value: "0.02",
  ...
  it.each([
    ...
    "makes current attribution unavailable for %s",
    (reason, missingInput) => {
      const { calculateUnrealizedAttribution } = loadAttributionApi();
  ...
  it.each([
    ...
    "keeps current combined P/L while the breakdown reports %s",
    (reason, missingInput) => {
      const { calculateUnrealizedAttribution } = loadAttributionApi();
  ...
  it.each([
    ...
    "makes realized attribution unavailable for %s",
    (reason, missingInput) => {
      const { calculateRealizedAttribution } = loadAttributionApi();
  ...
  it.each([
    ...
    "keeps realized combined P/L while the breakdown reports %s",
    (reason, missingInput) => {
      const { calculateRealizedAttribution } = loadAttributionApi();
